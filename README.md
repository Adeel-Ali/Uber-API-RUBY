...
Alternatively, you can build and install the gem manually:
  ...
  2. Run the build command: `gem build uber_api.gemspec`
  3. Run the install command: `gem install ./uber_api-1.0.gem`
...
After having installed the gem, you can easily use the SDK following these steps.
  ...
  1. Create a "uber_api_test.rb" file in the root directory.
  2. Use any controller as follows:
```ruby
require 'uber_api'

api_client = UberApi::UberAPIClient.new
controller = api_client.api_controller
response = controller.get_promotions(<required parameters if any>)
p response
```